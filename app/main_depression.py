...
def detect_and_store(method, approach, filenames, feature_acro, dr):
    '''
...
def early_detect_and_store(method, approach, filenames, feature_acro, dr):
    '''
...
def detect_depression(filenames, feature_acro, early_choice):
    '''
    ...
    for model in All_models:
        ...
        if len(model) > 2:
            ...
            if early_choice == 1:
                dr="early_detect"
                if not os.path.exists(os.path.join(output_dir,'test',str(feature_acro),str(method),str(approach),dr)):
                    os.makedirs(os.path.join(output_dir,'test',str(feature_acro),str(method),str(approach),dr))
                early_detect_and_store(method,approach,filenames,feature_acro, dr)
                ...
                detect_and_store(method, approach, filenames, feature_acro, dr)
...
if __name__ == '__main__':
    ...
    if detect:
        ...
        detect_depression(test_filenames, feature_acro, early_choice)
