...
def get_data_files(folder_path):
    ...
    '''
    #folder_path = fos.path.join(features_dir, sub_folder)
...
if __name__ == '__main__':
    ...
    if train:
        ...
        for m_idx in range(0, len(meta_groups)):
            ...
            meta_data_frame = train_data_frame.drop(train_data_frame.columns[meta_group], axis=1)
            Xtrain, Ytrain = get_features_labels(meta_data_frame)

            feature_acro = feature_all + "_" + meta_group_name + "_" + "exclusion"
            #training models
            for model in All_models:
        ...
        for g_idx in range(0, len(groups)):
            ...
            group_data_frame = train_data_frame.drop(train_data_frame.columns[group], axis=1)
            Xtrain, Ytrain = get_features_labels(group_data_frame)

            feature_acro = feature_all + "_" + group_name + "_" + "exclusion"
            #training models
            for model in All_models:
                train_model(Xtrain, Ytrain, model, feature_acro)
    ...
    if test:
        ...
        for filename in test_filenames:
            ...
            for m_idx in range(0, len(meta_groups)):
                meta_group = meta_groups[m_idx]
                meta_group_name = meta_group_names[m_idx]
                meta_data_frame = test_data_frame.drop(test_data_frame.columns[meta_group], axis=1)
            ...
            for g_idx in range(0, len(groups)):
                group = groups[g_idx]
                group_name = group_names[g_idx]
                group_data_frame = test_data_frame.drop(test_data_frame.columns[group], axis=1)
