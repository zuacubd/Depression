...
def prob_detect_and_store(method, approach, filenames, feature_acro, dr):
    '''
    ...
    output_filepath = os.path.join(output_dir,'test',feature_acro,method,approach,dr,filename[:-4]+".txt")
    ...
    for fileno in range(1, len(filenames)):
        filename = filenames[fileno]
        output_filepath = os.path.join(output_dir,'test',feature_acro,method,approach,dr,filename[:-4]+".txt")
        prediction_filepath = os.path.join(prediction_dir,'test',feature_acro,method,approach,filename[:-4]+".txt")
    ...
    #start working on the df_all to decide either depression or not
    uids = df_all['uids']
	...
	user_score = list(zip(uids, scores))
	results = pd.DataFrame(user_score)
	#storing results to the file
    results.to_csv(output_filepath, index=False, header=None, sep='\t')


def prob_early_detect_and_store(method, approach, filenames, feature_acro, combined, dr):
    '''
...
def detect_and_store(method, approach, filenames, feature_acro, combined, dr):
    '''
        detecting the depression considering all the chunks together and storing the results
    '''
    #1-9th (first 9th) chunk of data
    for fileno in range(0, len(filenames)-1):
        #prediction_file = open(os.path.join(prediction_dir,'test',feature_acro,filename[:-4]+"_"+approach+".txt"),'r')
        filename = filenames[fileno]
        output_filepath = os.path.join(output_dir,'test',feature_acro,method,approach,combined,dr,filename[:-4] + ".txt")
        prediction_filepath = os.path.join(prediction_dir,'test',feature_acro,method,approach,filename[:-4] + ".txt")
        data = pd.read_csv(prediction_filepath, sep='\t', header=None)
...
def early_detect_and_store(method, approach, filenames, feature_acro, combined, dr):
    '''
        detecting the early depression considering the different chunks and storing the results
    '''
    #1-9th (first 9th) chunk of data
...
def detect_depression(filenames, f_acro, early_choice, cumulative):
    '''
    ...
    for model in All_models:
        ...
        if len(model) > 2:
            ...
            if cumulative == 1:
                if not os.path.exists(os.path.join(output_path,str(method),str(approach),str(cumulative))):
                    os.makedirs(os.path.join(output_path,str(method),str(approach),str(cumulative)))

                if early_choice == 1:
                    dr="early_detect"
                    if not os.path.exists(os.path.join(output_path,str(method),str(approach),str(cumulative),dr)):
                        os.makedirs(os.path.join(output_path,str(method),str(approach),str(cumulative),dr))
                ...
                if early_choice == 1:
                    dr="early_detect"
                    if not os.path.exists(os.path.join(output_path,str(method),str(approach),dr)):
                        os.makedirs(os.path.join(output_path,str(method),str(approach),dr))
                    prob_early_detect_and_store(method,approach,filenames,f_acro dr)

                else:
                    dr="detect"
                    if not os.path.exists(os.path.join(output_path,,str(method),str(approach),dr)):
                        os.makedirs(os.path.join(output_path,str(method),str(approach),dr))
                    prob_detect_and_store(method,approach,filenames,feature_acro,dr)
...
if __name__ == '__main__':
    ...
    print ("Combining choice: {}".format(combined))
    ...
    if detect:
        ...
        detect_depression(test_filenames, feature_acro, early_choice, cumulative)
