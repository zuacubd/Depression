...
from feature_ablation_config import *
...
def get_data_files(folder_path):
    ...
    '''
    #folder_path = os.path.join(features_dir, sub_folder)
...
def detect_and_store(method, approach, filenames, feature_acro):
    '''
        detecting the depression considering all the files and storing the results
    '''
    ...
    for fileno in range(0, len(filenames)-1):
        ...
        output_filepath = os.path.join(output_dir, 'test', feature_acro, method, approach, filename[:-4] + ".txt")
        prediction_filepath = os.path.join(prediction_dir, 'test', feature_acro, method, approach, filename[:-4] + ".txt")
...
def detect_depression(filenames, feature_acro):
    '''
    ...
    for model in All_models:
        method = model[0]
        if not os.path.exists(os.path.join(output_dir, 'test', str(feature_acro), str(method))):
            os.makedirs(os.path.join(output_dir, 'test', str(feature_acro), str(method)))
        ...
        if len(model) > 2:
            ...
            for adx in range(0, len(training_approaches)):
                ...
                if not os.path.exists(os.path.join(output_dir, 'test', str(feature_acro), str(method), str(approach))):
                    os.makedirs(os.path.join(output_dir, 'test', str(feature_acro), str(method), str(approach)))

                detect_and_store(method, approach, filenames, feature_acro)
...
if __name__ == '__main__':
    ...
    training_approaches = ['natural', 'oversampled', 'undersampled', 'combined_ou', 'balanced']
    ...
    if train:
        ...
        #meta groups ablation
        for m_idx in range(0, len(meta_groups)):
            meta_group = meta_groups[m_idx]
            meta_group_name = meta_group_names[m_idx]
            meta_data_frame = train_data_frame.filter(train_data_frame.columns[[0] + meta_group + [257]], axis=1)
            Xtrain, Ytrain = get_features_labels(meta_data_frame)

            feature_acro = feature_all + "_" + meta_group_name + "_" + "inclusion"
            #training models
            for model in All_models:
                train_model(Xtrain, Ytrain, model, feature_acro)

        #group level ablation
        for g_idx in range(0, len(groups)):
            group = groups[g_idx]
            group_name = group_names[g_idx]
            group_data_frame = train_data_frame.filter(train_data_frame.columns[[0] + group + [257]], axis=1)
            Xtrain, Ytrain = get_features_labels(group_data_frame)

            feature_acro = feature_all + "_" + group_name + "_" + "inclusion"
            #training models
            for model in All_models:
                train_model(Xtrain, Ytrain, model, feature_acro)
    ...
    if test:
        ...
        for filename in test_filenames:
            ...
            #meta group ablations
            for m_idx in range(0, len(meta_groups)):
                meta_group = meta_groups[m_idx]
                meta_group_name = meta_group_names[m_idx]
                meta_data_frame = test_data_frame.filter(test_data_frame.columns[[0] + meta_group + [257]], axis=1)

                feature_acro = feature_all + "_" + meta_group_name + "_" + "inclusion"
                Xtest, Ytest = get_features_labels(meta_data_frame)

                data_subject = meta_data_frame['uid']
                predict_testset(data_subject, Xtest, filename, feature_acro)

            #group level ablations
            for g_idx in range(0, len(groups)):
                group = groups[g_idx]
                group_name = group_names[g_idx]
                group_data_frame = test_data_frame.filter(test_data_frame.columns[[0] + group + [257]], axis=1)

                feature_acro = feature_all + "_" + group_name + "_" + "inclusion"
                Xtest, Ytest = get_features_labels(group_data_frame)

                data_subject = group_data_frame['uid']
                predict_testset(data_subject, Xtest, filename, feature_acro)
    ...
    if detect:
        ...
        for m_idx in range(0, len(meta_groups)):
            ...
            detect_depression(test_filenames, feature_acro)
